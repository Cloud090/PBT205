...
def start_receiving_messages(channel, queue_name):
    try:
        ...
        print("Channel is closed, unable to consume messages.")
...
def check_username(channel):
    ...
    def on_response(ch, method, props, body):
        ...
        if props.correlation_id == our_correlation_id:
            ...
            print(f"Received response: {response}, from correlation ID: {props.correlation_id}")
    ...
    print(f"Sending username check for: {username}")
    ...
    if not username_responses:
        print("No responses received. Assuming username is available.")
        username_valid = True
    else:
        username_valid = all(response == '1' for response in username_responses)

    if not username_valid:
        print("Username is already taken. Please restart and try another one.")
        shutdown_event.set()
        connection.close()
        sys.exit(1)
    else:
        print("Username is available. Joining the chat room.")
        send_message(channel, "chat.general", f"{username} has joined the chat room", is_system=True)
...
def handle_username_check(channel, method, properties, body):
    ...
    print(f"Received username check request for: {requested_username}")
    ...
    if properties.correlation_id != our_correlation_id:
        ...
        print(f"Sending response for username '{requested_username}': {response}")
        channel.basic_publish(
            exchange='',
        ...
        print("Ignoring our own username check request.")
...
def main():
    ...
    try:
        ...
        shutdown_event.set()
        print(f"\n{username} has exited the chat room")
        connection.close()
        sys.exit(0)
